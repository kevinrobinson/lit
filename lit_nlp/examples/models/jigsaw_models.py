...
class ToxicityClassifierModelConfig(object):
  """Config options for a JigsawModel."""
  # Preprocessing options
  max_seq_length: int = 128
  inference_batch_size: int = 32
  # Input options
  text_name: str = "sentence"
  label_name: str = "label"
  # Output options
  compute_grads: bool = True  # if True, compute and return gradients.


class ToxicityClassifierModel(lit_model.Model):
  """Toxicity classification model, using Keras/TF2 and Huggingface Transformers.

  This is a regression model for scoring the "toxicity" of comment text.  See
  ToxicityClassifierModelConfig for available options.
  ...
  def __init__(self,
               ...
               for_training=False,
               **config_kw):
    self.config = ToxicityClassifierModelConfig(**config_kw)
  ...
  def _preprocess(self, inputs: Iterable[JsonDict]) -> Dict[str, tf.Tensor]:
    segments = [(ex[self.config.text_name]) for ex in inputs]
  ...
  def _postprocess(self, output: Dict[str, np.ndarray]):
    ...
    output["tokens_" + self.config.text_name] = output["tokens"][slicer_a]

    # Gradients for each segment, individually.
    if self.config.compute_grads:
      output["token_grad_" +
             self.config.text_name] = output["input_emb_grad"][slicer_a]
