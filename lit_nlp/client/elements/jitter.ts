...
import {SelectionService, AppState, ClassificationService, ColorService, RegressionService} from '../services/services';
 ...
 */
@customElement('jitter-chart')
export class JitterChart extends ReactiveElement {
  ...
  @observable private plotWidth = JitterChart.maxPlotWidth;
  @observable private plotHeight = JitterChart.minPlotHeight;
  ...
  private getXScale(key: string) {
    ...
    return d3.scaleLinear().domain(scoreRange).range([
      0, this.plotWidth - JitterChart.plotLeftMargin
    ]);
  }
  ...
  private getYScale(key: string) {
    ...
    return d3.scaleLinear().domain(scoreRange).range([
      this.plotHeight - JitterChart.plotBottomMargin, 0
    ]);
  }
  ...
  private updateThreshold() {
    ...
    for (const item of Array.from(scatterplotClass)) {
      ...
      thresholdSelection.append('line')
          ...
          .attr(
              'y2',
              yScale(this.plotHeight - JitterChart.plotBottomMargin))
          .style('stroke', 'black');
    }
  }
  ...
  private makePlot() {
    ...
    this.plotWidth =
        container.offsetWidth - JitterChart.plotLeftMargin * 2;
    ...
    const proposedPlotHeight =
        (container.offsetHeight - JitterChart.plotBottomMargin) /
            scatterplots.length -
        perRowMargin;
    this.plotHeight = Math.min(
        Math.max(JitterChart.minPlotHeight, proposedPlotHeight),
        JitterChart.maxPlotHeight);
    ...
    for (const item of Array.from(scatterplots)) {
      ...
      d3.select(scatterplot)
          ...
          .attr(
              'transform',
              'translate(' + JitterChart.plotLeftMargin.toString() +
                  ',0)');
      this.updateThreshold();
    ...
    for (const item of Array.from(scatterplots)) {
      ...
      d3.select(scatterplot)
          ...
          .attr(
              ...
              'translate(' + JitterChart.plotLeftMargin.toString() +
                  ',' +
                  (this.plotHeight - JitterChart.plotBottomMargin)
                      .toString() +
                  ')')
      ...
      if (isScalarKey && hasRegressionGroundTruth) {
        d3.select(scatterplot)
            ...
            .attr(
                'transform',
                'translate(' +
                    (JitterChart.plotLeftMargin +
                     JitterChart.yLabelOffsetX)
                        .toString() +
                    ', ' +
                    (this.plotHeight / 2 + JitterChart.yLabelOffsetY)
                        .toString() +
                    ') rotate(270)')
            .style('text-anchor', 'middle')
            .text('error');
      ...
      if (isScalarKey && hasRegressionGroundTruth) {
        ...
        if (yRange[0] < 0 && yRange[1] > 0) {
          d3.select(scatterplot)
              .append('line')
              .attr('x1', JitterChart.plotLeftMargin)
              .attr(
                  'y1',
                  (this.plotHeight - JitterChart.plotBottomMargin) /
                      2)
              .attr('x2', this.plotWidth + JitterChart.plotLeftMargin)
              .attr(
                  'y2',
                  (this.plotHeight - JitterChart.plotBottomMargin) /
                      2)
              .style('stroke', JitterChart.zeroLineColor);
        }
      }
      ...
      newBrush
          .extent([
            [0, 0],
            [
              this.plotWidth - JitterChart.plotLeftMargin,
              this.plotHeight - JitterChart.plotBottomMargin
            ]
          ])
          .on('start end', () => {
      ...
      const brushGroup =
          selected.append('g')
              ...
              .attr(
                  'transform',
                  'translate(' +
                      JitterChart.plotLeftMargin.toString() + ',0)')
              .call(newBrush);
  ...
  render() {
    // clang-format off
    const key = 'default';
    const label = 'probabilities!';
    return html`
      <div id='container'>
        ${this.renderPlot(key, label)}
      </div>
    `;
    // clang-format on
  }
